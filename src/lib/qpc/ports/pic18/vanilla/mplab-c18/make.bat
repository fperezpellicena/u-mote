...
rem
set MPLAB_C18="C:\Archivos de programa\Microchip\mplabc18\v3.40"
set MPLAB_ASM="C:\Archivos de programa\Microchip\mplabc18\v3.40\mpasm"

rem adjust the following symbol to the PICmicro device you're using
rem
set PIC_MICRO=18f46j50
...
if "%1"=="" (
    ...
    set BINDIR=dbg
    set CCFLAGS=-p=%PIC_MICRO% -ms -I=%MPLAB_C18%\h --extended -w3 -Ou- -Ot- -Ob- -Op- -Or- -Od- -Opa- -nw=2055
)
if "%1"=="rel" (
    echo rel selected
    set BINDIR=rel
    set CCFLAGS=-p=%PIC_MICRO% -ms -I=%MPLAB_C18%\h --extended -w3 -nw=2055 -DNDEBUG
)
if "%1"=="spy" (
    echo spy selected
    set BINDIR=spy
    set CCFLAGS=-p=%PIC_MICRO% -ms -I=%MPLAB_C18%\h --extended -w3 -Ou- -Ot- -Ob- -Op- -Or- -Od- -Opa- -nw=2055 -nw=2060 -DQ_SPY
)
